...
class DBControl(object):
    def __init__(self, mq_ip='34.222.23.216', mq_port=61613):
        try:
            self.mq = stomp.Connection([(mq_ip, 61613)])
            self.mq.start()
            self.mq.connect(wait=True)

            self.user_ser = {}          # username : app server ip
            self.ser_usercount = {}     # ip : number of user
            self.ser_machine = {}       # ip : machine id
